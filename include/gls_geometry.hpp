...
struct basic_point {
    ...
    constexpr bool operator==(const basic_point& other) const { return x == other.x && y == other.y; }

    constexpr basic_point<T> operator+(const T& value) const {
        basic_point<T> result = {x + value, y + value};
        return result;
    }
    ...
    constexpr basic_point<T> operator+(const basic_point<T>& other) const {
        basic_point<T> result = {x + other.x, y + other.y};
        return result;
    }
    ...
    constexpr operator gls::Vector<2, TV>() const {
        return {(TV)x, (TV)y};
    }
...
struct basic_size {
    ...
    constexpr bool operator==(const basic_size& other) const { return width == other.width && height == other.height; }

    constexpr basic_size<T> operator+(const T& value) const {
        basic_size<T> result = {width + value, height + value};
        return result;
    }
...
struct basic_rectangle : public basic_point<T>, basic_size<T> {
    constexpr basic_rectangle(basic_point<T> _origin, basic_size<T> _dimensions)
        : basic_point<T>(_origin), basic_size<T>(_dimensions) {}
    constexpr basic_rectangle(basic_point<T> p1, basic_point<T> p2)
        : basic_point<T>(std::min(p1.x, p2.x), std::min(p1.y, p2.y)),
          basic_size<T>(std::abs(p1.x - p2.x), std::abs(p1.y - p2.y)) {}
    constexpr basic_rectangle(T _x, T _y, T _width, T _height)
        : basic_point<T>(_x, _y), basic_size<T>(_width, _height) {}
    constexpr basic_rectangle() {}

    constexpr bool contains(const gls::basic_point<T> p) const {
        return p.x >= basic_point<T>::x && p.y >= basic_point<T>::y && p.x < basic_point<T>::x + basic_size<T>::width &&
               p.y < basic_point<T>::y + basic_size<T>::height;
    }
...
struct std::hash<gls::size> {
    constexpr std::size_t operator()(gls::size const& r) const noexcept { return r.width ^ r.height; }
};

template <>
struct std::hash<gls::point> {
    constexpr std::size_t operator()(gls::point const& p) const noexcept { return p.x + p.y; }
};
